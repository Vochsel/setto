...
import { Clapperboard, MapPin, Camera, Calendar } from "lucide-react";
import {
  formatDateTime,
  shootStatusMeta,
  type ShootStatus,
...
export interface ShootCardData {
  _id: string;
  name: string;
  description?: string;
  status: ShootStatus;
...
/** Crossfades through the shoot's most recent photos. */
...
export function ShootCard({ shoot }: { shoot: ShootCardData }) {
  const status = shootStatusMeta[shoot.status];
  const photos = shoot.recentImages ?? [];
  const statusPill =
    shoot.status === "active"
  ...
  return (
    <Link href={`/shoots/${shoot._id}`} className="group block">
      <div className="dark:highlight-inset overflow-hidden rounded-xl border border-[#e4e4e7] bg-white shadow-soft transition-transform group-hover:-translate-y-0.5 dark:border-[#1e2c31] dark:bg-[#0a0a0a] dark:shadow-none">
        {/* Photo frame — full-bleed slideshow, no inner padding */}
        <div className="relative aspect-[4/5] overflow-hidden bg-[#f0f0ea] dark:bg-black">
          {photos.length ? (
            ...
            <div className="flex h-full items-center justify-center">
              <Clapperboard className="h-8 w-8 text-black/15 dark:text-white/20" />
            </div>
          )}
          <span
            className={cn(
        ...
        <div className="p-4">
          ...
          <p className="mt-1 flex items-center gap-1.5 text-[13px] text-[#71717a] dark:text-[#a1a1aa]">
          ...
          <div className="mt-3 flex items-center gap-4 text-[13px] text-[#71717a] dark:text-[#a1a1aa]">
            <span className="flex items-center gap-1">
              <MapPin className="h-3 w-3" /> {shoot.locationCount ?? 0} locations
            </span>
            <span className="flex items-center gap-1">
              <Camera className="h-3 w-3" /> {shoot.shotCount ?? 0} shots
            </span>
          </div>
        </div>
      </div>
